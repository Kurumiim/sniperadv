		...
		<table id="cabecalhotabela" style="border-bottom-style: solid; font-size: 8px; margin-bottom:10px; width: 100%; table-layout: fixed;">
			<thead>
				...
				<tr id="cabeca" style="margin-left: 0px; background-color:#CCC">
					...
					<td style="width:13%">RECEBEDOR</td>
...
if($mostrar_registros == 'Não'){
	if($pago == 'Vencidas'){
		$query = $pdo->query("SELECT * from receber where vencimento < curDate() and pago != 'Sim' and usuario_receb = '$id_usuario' and financeiro is null order by id desc");
	}else{
	$query = $pdo->query("SELECT * from receber where $tipo_data >= '$dataInicial' and $tipo_data <= '$dataFinal' and pago LIKE '%$pago%' and usuario_receb = '$id_usuario' and financeiro is null order by id desc");
	}
}else{
	if($pago == 'Vencidas'){
		$query = $pdo->query("SELECT * from receber where vencimento < curDate() and pago != 'Sim' and financeiro is null order by id desc");
	}else{
	$query = $pdo->query("SELECT * from receber where $tipo_data >= '$dataInicial' and $tipo_data <= '$dataFinal' and pago LIKE '%$pago%' and financeiro is null order by id desc");
	}
}
...
for($i=0; $i<$linhas; $i++){
	...
	$texto_parcela = '';
	if($parcela != ""){
		$texto_parcela = '<span style="font-size:7px; color:red">(Parcela '.$parcela.')</span>';
	}
...
<img style="margin-top: 0px" src="<?php echo $url_sistema ?>painel/images/<?php echo $classe_pago ?>" width="8px">
	<?php echo $descricao ?> <?php echo $texto_parcela ?></td>
...
<td style="width:13%"><?php echo $nome_usu_receb ?></td>
<td style="width:10%"><?php echo $nome_pgto ?></td>
<td style="width:10%"><?php echo $nome_frequencia ?></td>
